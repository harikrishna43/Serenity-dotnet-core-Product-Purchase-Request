...
using MyRoleRow = ProductPurchaseApp.Administration.UserRoleRow;

namespace ProductPurchaseApp.AppServices;
public class UserRetrieveService(ITwoLevelCache cache, ISqlConnections sqlConnections) : IUserRetrieveService
{
    ...
    private static bool GetRoles(IDbConnection connection, BaseCriteria criteria)
    {
        var userRole = connection.TryById<MyRoleRow>(criteria);
        return true;//userRole.RoleName;
    }
    public bool RoleById(string id)
    {
        using var connection = SqlConnections.NewByKey("Default");
            return GetRoles(connection, new Criteria(Fld.UserId) == int.Parse(id, CultureInfo.InvariantCulture));
    }
